...
static void tx_buffer_process(void)
{
    if (m_tx_index != m_tx_insert_index)
    {
        ...
        {
						LOG("[HRS_C]: sd_ble_gattc_read(%d, %d) //JHG\r\n", m_tx_buffer[m_tx_index].conn_handle,m_tx_buffer[m_tx_index].req.read_handle);
        ...
        {
						LOG("[HRS_C]: sd_ble_gattc_write(%d, %s)\r\n", m_tx_buffer[m_tx_index].conn_handle,m_tx_buffer[m_tx_index].req.write_req.gattc_value);
...
uint32_t ble_hrs_c_init(ble_hrs_c_t * p_ble_hrs_c, ble_hrs_c_init_t * p_ble_hrs_c_init)
{
    ...
    mp_ble_hrs_c->hrm_cccd_handle = BLE_GATT_HANDLE_INVALID;
		...
		LOG("[HRS_C]: ble_db_discovery_evt_register(BLE_UUID_HEART_RATE_SERVICE : 0x%x, uuid : 0x%02x)\r\n",BLE_UUID_HEART_RATE_SERVICE, hrs_uuid.uuid );
...
uint32_t ble_hrs_c_hrm_notif_enable(ble_hrs_c_t * p_ble_hrs_c)
{
    if (p_ble_hrs_c == NULL)
    {
		...
		LOG("[HRS_C]: ble_hrs_c_hrm_notif_enable //JHG\r\n");
